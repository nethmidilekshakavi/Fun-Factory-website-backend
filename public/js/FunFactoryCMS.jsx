const { useState, useCallback } = React;
       ...
       const SECTIONS = [
         ...
         { key: "footer",    label: "Footer",    icon: "📄" }, // ✅ FIX: footer added
       ];

       function Badge({ children, color = "blue" }) {
         const colors = {
           blue: { bg: "#e0edff", text: "#1a5fa8" },
           green: { bg: "#e0f5ee", text: "#0f6e56" },
           yellow: { bg: "#fef9e0", text: "#7a5000" },
       ...
       function Field({ label, value, onChange, multiline, hint }) {
         return (
           <div style={{ marginBottom: 14 }}>
             ...
             {hint && (
               <p style={{ fontSize: 11, color: "#888", margin: "4px 0 0" }}>{hint}</p>
             )}
           </div>
         );
       }
       ...
       function NavbarEditor({ data, onChange }) {
         ...
         const addLink = () =>
           onChange({ ...data, links: [...data.links, { label: "New Link", href: "#" }] });
         const removeLink = (i) => {
         ...
         return (
           <SectionCard title="Navigation Bar">
             <div style={{ marginBottom: 14 }}>
               <label style={{ fontSize: 12, fontWeight: 600, color: "#555", textTransform: "uppercase", letterSpacing: 0.3 }}>
                 Nav Links
               </label>
             ...
             {/* Logo Image Upload */}
             <div style={{ marginBottom: 14 }}>
               <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#555", marginBottom: 8, letterSpacing: 0.3, textTransform: "uppercase" }}>
                 Logo Image
               </label>

               {/* Preview */}
               <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                 <div style={{ width: 80, height: 56, borderRadius: 8, border: "1.5px solid #d0e0f0", background: "#f4f8fc", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", flexShrink: 0 }}>
                   {data.logoSrc
                     ? <img src={data.logoSrc} alt="Logo" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain" }} onError={(e) => { e.target.style.display="none"; }} />
                     : <span style={{ fontSize: 11, color: "#aaa" }}>No logo</span>
                   }
                 </div>
                 <div style={{ flex: 1 }}>
                   <div style={{ fontSize: 12, fontWeight: 700, color: "#333", marginBottom: 4 }}>{data.logoSrc ? data.logoSrc.split("/").pop() : "No file selected"}</div>
                   <div style={{ fontSize: 11, color: "#888" }}>PNG, JPG, SVG — saved to <code>public/images/</code></div>
                 </div>
               </div>

               {/* Upload button (hidden input trick) */}
               <label style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#e0edff", color: "#1a5fa8", border: "none", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                 📁 Choose Image
                 <input
                   type="file"
                   accept="image/*"
                   ...
                   onChange={async (e) => {
                     ...
                     fd.append("logo", file);
                     try {
                       const res = await fetch("/api/cms/upload-logo", { method: "POST", body: fd });
                       const json = await res.json();
                       if (json.path) {
                         onChange({ ...data, logoSrc: json.path });
                       } else {
               ...
               <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                 <input
                   type="text"
                   value={data.logoSrc || ""}
                   onChange={(e) => onChange({ ...data, logoSrc: e.target.value })}
                   placeholder="or type path manually: images/logo.png"
                   style={{ flex: 1, border: "1.5px solid #d0e0f0", borderRadius: 8, padding: "7px 10px", fontSize: 12, outline: "none", background: "#fff", color: "#555" }}
                 />
                 {data.logoSrc && (
                   <button onClick={() => onChange({ ...data, logoSrc: "" })} style={{ background: "#fce8e8", color: "#a32d2d", border: "none", borderRadius: 8, padding: "7px 12px", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>✕</button>
                 )}
               </div>
             </div>
           ...
           <SectionCard title="Locations">
             {data.map((loc, i) => (
               <div key={i} style={{ marginBottom: i < data.length - 1 ? 18 : 0, paddingBottom: i < data.length - 1 ? 18 : 0, borderBottom: i < data.length - 1 ? "1px dashed #e0eaf5" : "none" }}>
                 <div style={{ fontSize: 12, fontWeight: 700, color: "#1a5fa8", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>
                   Branch {i + 1}
                 </div>
       ...
       // ✅ FIX: FooterEditor component added
           ...
           <div style={{ fontFamily: "Nunito, sans-serif", fontSize: 13, color: "#222" }}>
             ...
             >
               <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                 {data.navbar.logoSrc
                   ? <img src={data.navbar.logoSrc} alt="Fun Factory" style={{ height: 40, width: "auto", objectFit: "contain" }} onError={(e) => { e.target.style.display="none"; }} />
       ...
       function FunFactoryCMS() {
         ...
         const [activeTab, setActiveTab] = useState("edit"); // edit | preview | code
         ...
         const handleSave = async () => {
           // ✅ FIX: Laravel API ෙකට save කරනවා, fail ෙවෙනෙකොට localStorage fallback
           try {
             ...
             // API neti unama localStorage fallback
             try {
               localStorage.setItem('ff_cms_data', JSON.stringify(data));
             } catch(le) {
               alert('Save failed: ' + le.message);
               return;
             }
           }
           setSaved(true);
         ...
         const handleAI = async () => {
           ...
           setAiLoading(true);
           try {
             const res = await fetch("/api/cms/ai-edit", { // ✅ FIX: correct route
               method: "POST",
               headers: { "Content-Type": "application/json" },
               body: JSON.stringify({
                 messages: [
             ...
             updateSection(activeSection, parsed);
             setAiPrompt("");
           } catch (e) {
             alert("AI edit failed. Check console.");
             console.error(e);
           }
           setAiLoading(false);
         };
         ...
         const renderEditor = () => {
           switch (activeSection) {
             ...
             case "footer":    return <FooterEditor    data={data.footer}    onChange={(v) => updateSection("footer", v)} />; // ✅ FIX
             default: return null;
           }
         };
         ...
         return (
           <div style={{ display: "flex", height: "100vh", fontFamily: "Nunito, sans-serif", background: "#f4f8fc", overflow: "hidden" }}>
             ...
             <div style={{ width: 210, background: "#1a3a6b", display: "flex", flexDirection: "column", flexShrink: 0, overflowY: "auto" }}>
               ...
               <div style={{ flex: 1, padding: "6px 10px" }}>
                 {SECTIONS.map((s) => (
                   ...
                   >
                     ...
                     {s.label}
                     {activeSection === s.key && (
                       <span style={{ marginLeft: "auto", width: 6, height: 6, borderRadius: "50%", background: "#F7C416", flexShrink: 0 }} />
                     )}
                   </button>
                 ))}
               </div>
               ...
               <div style={{ padding: "14px 10px 18px" }}>
                 <button
                   ...
                   style={{
                     width: "100%",
                     background: saved ? "#0f6e56" : "#F7C416",
             ...
             <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
               ...
               {/* AI BAR */}
               <div style={{ background: "linear-gradient(90deg,#e8f0ff,#f0e8ff)", borderBottom: "1.5px solid #d0d0f0", padding: "10px 24px", display: "flex", gap: 10, alignItems: "center", flexShrink: 0 }}>
                 <span style={{ fontSize: 14 }}>🤖</span>
                 <input
                   value={aiPrompt}
                   onChange={(e) => setAiPrompt(e.target.value)}
                   onKeyDown={(e) => e.key === "Enter" && handleAI()}
                   placeholder={`Ask AI to edit "${SECTIONS.find(s=>s.key===activeSection)?.label}"... e.g. "make the banner more exciting" or "add a Membership link"`}
                   style={{
                     flex: 1,
                 ...
                 <button
                   ...
                   style={{
                     background: aiLoading ? "#888" : "#534AB7",
                     color: "#fff",
                     border: "none",
                     borderRadius: 8,
                     padding: "8px 18px",
                 ...
                 {activeTab === "code" && (
                   <div style={{ padding: 20 }}>
                     <div style={{ marginBottom: 12, fontSize: 13, color: "#555", fontWeight: 600 }}>
                       Copy this HTML and paste it into your <strong>index.html</strong> to update the relevant sections:
                     </div>
